...
<!--                    <tr>
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="box box-solid box-primary">
                    <form action="<?php echo base_url(); ?>index.php/<?php echo $this->router->class; ?>/updatePositionIDExc" method="post" enctype="multipart/form-data" id="form2" name="form2">
                        <div class="box-header">
                            <h3 class="box-title">กำหนดประธานรุ่น (เฉพาะผู้ที่ผ่านการอนุมัติ)</h3>
                        ...
                        <div class="box-body table-responsive no-padding">
                            <table id="example1" class="table table-bordered table-hover">
                                <tr>
                                    <th style="width: 20%">เลขรหัสประชาชน</th>
                                    <th style="width: 20%">ชื่อ-นามสกุล</th>
                                    <th style="width: 20%">เลขที่วุฒิบัตร</th>
                                ...
                                if ($history != NULL) {
                                    foreach ($history as $row) {
                                        ...
                                        echo '  <select class="form-control" name="POSITION_ID_' . $row['HISTORY_ID'] . '">';
                                        if ($row['POSITION_ID'] == '') {
                                            echo '      <option value="" selected="true">ไม่ระบุ</option>';
                                        } else {
                                            echo '      <option value="">ไม่ระบุ</option>';
                                        }
                                        foreach ($position as $rd) {
                                            if ($rd['POSITION_ID'] == $row['POSITION_ID']) {
                                                echo '<option selected="true" value="' . $rd['POSITION_ID'] . '">' . $rd['POSITION_NAME'] . '</opoption>';
                                            } else {
                                                echo '<option value="' . $rd['POSITION_ID'] . '">' . $rd['POSITION_NAME'] . '</opoption>';
                                            }
                                        }
                                        echo '  <select>';
                                        echo '  </td>';
                                    ...
                                    <td colspan="9">
                                        <div class="box-footer clearfix">
                                            <div class="pull-right">
                                                <?php
                                                $config['base_url'] = base_url() . 'index.php/' . $this->router->class . '/updatePositionID/';
